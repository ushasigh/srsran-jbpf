...
def read_from_db(db_file):
    ...
    for line_number, line in enumerate(db_file, start=1):
        print(f"Line {line_number}: {line.strip()}")  # Print the line being processed
        if line.startswith("#") or not line.strip():  # Ignore comments and blank lines
            continue

        try:
            name, imsi, key, op_type, op_c, amf, qci, ip_alloc = line.split(',')
        except ValueError as e:
            print(f"Error reading subscriber_db.csv on line {line_number}: {e}")
            continue  # Skip the problematic line but continue processing others
    ...
    if not subscriber_db:
        print("No valid subscriber data found in the file.")
        return None
