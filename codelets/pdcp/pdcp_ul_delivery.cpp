...
jbpf_ringbuf_map(pdcp_ul_delivery_output_map, pdcp_ul_delivery, 256);

struct jbpf_load_map_def SEC("maps") output_map_tmp = {
  .type = JBPF_MAP_TYPE_ARRAY,
  .key_size = sizeof(int),
  .value_size = sizeof(pdcp_ul_delivery),
  .max_entries = 1,
};
...
uint64_t jbpf_main(void* state)
{
    ...
    jbpf_printf_debug("pdcp_ul_delivery\n");
    ...
    pdcp_ul_delivery *out = (pdcp_ul_delivery *)jbpf_map_lookup_elem(&output_map_tmp, &zero_index);
    if (!out)
    ...
    */
    ...
    int ret = jbpf_ringbuf_output(&pdcp_ul_delivery_output_map, (void *)out, sizeof(pdcp_ul_delivery));
    jbpf_map_clear(&output_map_tmp);
    if (ret < 0) {
#ifdef DEBUG_PRINT
        jbpf_printf_debug("pdcp_ul_delivery: Failure: jbpf_ringbuf_output\n");
#endif
        return JBPF_CODELET_FAILURE;
    }
