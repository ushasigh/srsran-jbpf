...
jbpf_ringbuf_map(pdcp_dl_new_sdu_output_map, pdcp_dl_new_sdu, 256);

struct jbpf_load_map_def SEC("maps") output_map_tmp = {
  .type = JBPF_MAP_TYPE_ARRAY,
  .key_size = sizeof(int),
  .value_size = sizeof(pdcp_dl_new_sdu),
  .max_entries = 1,
};
...
uint64_t jbpf_main(void* state)
{
    ...
    pdcp_dl_new_sdu *out = (pdcp_dl_new_sdu *)jbpf_map_lookup_elem(&output_map_tmp, &zero_index);
    if (!out)
    ...
    */
    ...
    if (ret < 0) {
#ifdef DEBUG_PRINT
        jbpf_printf_debug("pdcp_dl_new_sdu: Failure: jbpf_ringbuf_output\n");
#endif
        return JBPF_CODELET_FAILURE;
    }
