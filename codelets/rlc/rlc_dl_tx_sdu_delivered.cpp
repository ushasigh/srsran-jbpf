...
uint64_t jbpf_main(void* state)
{
...
#ifdef DEBUG_PRINT
    jbpf_printf_debug("RLC DL TX SDU DELIVERED: du_ue_index=%d, rb_id=%d, pdu_length=%d\n", 
    ...
    /////////////////////////////////////////////
    // sdu_tx_delivered
    out->stats[ind % MAX_NUM_UE_RB].sdu_tx_delivered.count++;
    if (latency_ns < out->stats[ind % MAX_NUM_UE_RB].sdu_tx_delivered.min) {
        out->stats[ind % MAX_NUM_UE_RB].sdu_tx_delivered.min = latency_ns;
    }
    if (latency_ns > out->stats[ind % MAX_NUM_UE_RB].sdu_tx_delivered.max) {
        out->stats[ind % MAX_NUM_UE_RB].sdu_tx_delivered.max = latency_ns;
    }
    out->stats[ind % MAX_NUM_UE_RB].sdu_tx_delivered.total += latency_ns;
