...
struct jbpf_load_map_def SEC("maps") output_map_tmp = {
  ...
  .key_size = sizeof(int),
  .value_size = sizeof(rrc_ue_procedure),
  .max_entries = 1,
};
...
uint64_t jbpf_main(void* state)
{
    ...
    rrc_ue_procedure *out = (rrc_ue_procedure *)jbpf_map_lookup_elem(&output_map_tmp, &zero_index);
    if (!out)
        return JBPF_CODELET_FAILURE;

    out->timestamp = jbpf_time_get_ns();
    out->cucp_ue_index = rrc_ctx.cu_cp_ue_index;
    out->procedure = ctx->srs_meta_data1 >> 32;
    out->success = (ctx->srs_meta_data1 >> 16) & 0xFFFFFFFF;
    out->meta = ctx->srs_meta_data1 & 0xFFFFFFFF;

    int ret = jbpf_ringbuf_output(&rrc_ue_procedure_output_map, (void *)out, sizeof(rrc_ue_procedure));
    jbpf_map_clear(&output_map_tmp);
    if (ret < 0) {
#ifdef DEBUG_PRINT
        jbpf_printf_debug("rrc_ue_procedure: Failure: jbpf_ringbuf_output\n");
#endif
        return JBPF_CODELET_FAILURE;
    }
