...
uint64_t jbpf_main(void* state)
{
    ...
    // get the output map
    int zero_index=0;
    du_ue_ctx_creation *out = (du_ue_ctx_creation *)jbpf_map_lookup_elem(&output_tmp_map, &zero_index);
    if (!out) {
    ...
    out->tac = du_ctx.tac;
    out->plmn = du_ctx.plmn;
    out->nci = du_ctx.nci;
    out->pci = du_ctx.pci;
    out->crnti = du_ctx.crnti;
        
    int ret = jbpf_ringbuf_output(&output_map, (void *)out, sizeof(du_ue_ctx_creation));
    
    if (ret < 0) {
#ifdef DEBUG_PRINT
        jbpf_printf_debug("du_ue_ctx_creation: Failure: jbpf_ringbuf_output\n");
#endif
        return JBPF_CODELET_FAILURE;
    }
