...
uint64_t jbpf_main(void* state)
{
    ...
    // get the output map
    int zero_index=0;
    cucp_ue_ctx_deletion *out = (cucp_ue_ctx_deletion *)jbpf_map_lookup_elem(&output_tmp_map, &zero_index);
    if (!out) {
    ...
    int ret = jbpf_ringbuf_output(&output_map, (void *)out, sizeof(cucp_ue_ctx_deletion));
    
    if (ret < 0) {
#ifdef DEBUG_PRINT
        jbpf_printf_debug("cucp_ue_ctx_deletion: Failure: jbpf_ringbuf_output\n");
#endif
        return JBPF_CODELET_FAILURE;
    }
