...
jbpf_ringbuf_map(output_map, dl_config_stats, 1000);
...
uint64_t jbpf_main(void *state)
{
    ...
    if (!c)
        return JBPF_CODELET_FAILURE;
    dl_config_stats *out = (dl_config_stats *)c;
    ...
    if (*not_empty_hist && *last_timestamp < timestamp32)
    {
        ...
        int ret = jbpf_ringbuf_output(&output_map, (void *) out, sizeof(dl_config_stats));
        ...
        *last_timestamp = timestamp32;
        ...
        if (ret < 0) {
#ifdef DEBUG_PRINT
            jbpf_printf_debug("fapi_gnb_dl_config_stats_report: Failure: jbpf_ringbuf_output\n");
#endif
            return JBPF_CODELET_FAILURE;
        }
